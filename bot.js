...
function flee() {
  ...
  // Run away 25 blocks in opposite direction from threat
  bot.pathfinder.setGoal(
    new GoalNear(pos.x + (dx / len) * 25, pos.y, pos.z + (dz / len) * 25, 2)
  );
  ...
  const check = setInterval(() => {
    ...
    // Safe now → idle
    if (!getNearestEntity(HOSTILE_MOBS, 10) && bot.health >= HP.SAFE) {
      clearInterval(check);
      bot.pathfinder.setGoal(null);
